			...
			req.on('end',function() {
				new SkyPlusHDXmlParser(chunks).then(function(result) {
					new SkyPlusHDXmlParser(result['e:propertyset']['e:property'].LastChange).then(function(results) {
						var ev = {};
						ev.playSpeed = +results.Event.InstanceID.TransportPlaySpeed.$.val;
						ev.uri = results.Event.InstanceID.CurrentTrackURI.$.val;
						//
						if (ev.uri.indexOf('xsi://') === 0) {
							ev.source = 'broadcast'
							ev.uri_idHex = ev.uri.substr('xsi://'.length) ;
							ev.uri_id = parseInt(ev.uri_idHex,16);
						} else if (ev.uri.indexOf('file://') === 0) {
							ev.source = 'pvr'
							ev.uri_idHex = ev.uri.substr('file://'.length) ;
							ev.uri_id = parseInt(ev.uri_idHex,16);
						} else {
							ev.source = 'other'
						};
