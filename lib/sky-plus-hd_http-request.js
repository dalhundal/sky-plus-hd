...
SkyPlusHDHttpRequest.soapRequest = function(serviceUrl, service, method, payload) {
	return SkyPlusHDHttpRequest.device({
		...
		try {
			var methodResponse = response.body['s:Envelope']['s:Body'][util.format('u:%sResponse',method)];
			response.payload = {};
			...
			return q.reject("Failed to parse response payload");
		}
	});
};
