 ...
 */
var SkyPlusHDHttpRequest = module.exports.SkyPlusHDHttpRequest = function(params, usingRequest) {
	var deferred = q.defer();
...
SkyPlusHDHttpRequest.getJson = function(params, usingRequest) {
	var deferred = q.defer();
	SkyPlusHDHttpRequest(params, usingRequest).then(function(response) {
		try {
...
SkyPlusHDHttpRequest.device = function(params) {
	var deferred = q.defer();
	SkyPlusHDHttpRequest(params, _SkyPlusHDHttpRequestDevice).then(function(response) {
		SkyPlusHDXmlParser(response.body).then(function(parsedBody) {
			deferred.resolve({
				body: parsedBody,
				headers: response.headers
			});	
...
var SkyPlusHDBox = module.exports.SkyPlusHDBox = function(params) {
	...
	Object.defineProperty(self,'supportsPhotoViewing',{
		get: function() {
			return (
				(parseInt(details.software.match(/^R(\d+)\./)[1]) >= 10)
				&&
				(details.modelDescription.match(/^4F31/i))
			)
		}
	})
	...
	this.showImage = function(url) {
		if (!self.supportsPhotoViewing) {
			...
			return SkyPlusHDHttpRequest(util.format(
